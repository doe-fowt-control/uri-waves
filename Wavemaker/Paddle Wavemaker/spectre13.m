...
%=========================================================================
% PM: FETCH = -1; enter U10=0 AMPS = 0 => use OMEGAP, find U10 and AMPS 
%                                  > 0 => use AMPS, find U10 and OMEGAP
%                       U10>0 AMPS = 0 => use  U10, find OMEGAP and AMPS 
%                              
% JS: FETCH >0  U10>0     => use U10 & FETCH,    find OMEGAP, ALPHA, AMPS 
%     FETCH =0  U10=0     => use OMEGAP & AMPS,  find U10,FETCH 
%     FETCH >0  U10=0     => use OMEGAP & FETCH, find U10, AMPS   
%      
...
%     AMPS: significant wave amplitude = Hs/2
%     OMEGAP: Peak specctral angular frequency
%     JSGAM: steep ness parameter for JS spectrum
%     GE: gravity
%     U10: wind speed at 10 m
%     EPSSP: fraction of maximum specrtal peak for low/high freq. truncation
%     NOMSP: Specified number of frequencies used in spectrum definition
%     [OMEGMIN,OMEGMAX] : constraint on maximum/min freq. from wavemaker
%
%     Return arguments: vectors (NOMSP dimens.) of [AMPN,PSIN,OMEGAN]
%     wave harmonic/componenet (amplitude, random phase, ang. frequency)
%     Note, random free surface at t is: ETA = sum(AMPN.*cos(OMEGAN.*t + PSIN))
%=========================================================================
...
% Find frequency interval [OMSMIN,OMSMAX] with constraint [OMEGMIN,OMEGMAX]
% for spectrum > EPS its maximum 

[Smax,iSmax]  = max(SWAVEL);
ismall        = find(SWAVEL >= Smax*EPSSP);
OMSMIN        = max([min(OMEGAM(ismall)) OMEGMIN]);  % Makes sure spectrum does not exceed tug cover
OMSMAX        = min([max(OMEGAM(ismall)) OMEGMAX]);
