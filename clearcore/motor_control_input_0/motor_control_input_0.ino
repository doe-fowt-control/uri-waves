...
int sendFlag = 0;
  ...
  // boolean flags set to true when limit is reached during homing routine
bool home0 = false;
bool home1 = false;
...
// PASSIVE
  if ( state == 2 ) {
    ...
    if (motor.StatusReg().bit.AlertsPresent) {
        Serial.println("Motor status: 'In Alert'");
...
// MOVING
  if ( state == 3 ) {
    ...
    double desiredPos = ( inputVoltage - 3 ) * 800;
    /*
    if ( !started ) {
      startTimeSec = millis() * 0.001;
      //SerialPort.Send("start time in IF: ");
    ...
    float t = currentTimeSec - startTimeSec;
